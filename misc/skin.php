...
class skin {
  ...
  function send_email($email) {
    ...
    mail($email, iconv('CP1251', 'UTF-8', $subject), iconv('CP1251', 'UTF-8', $body), "From: {$CONF['your_email']}\r\nContent-Type: text/html; charset=utf-8");
  }
...
class main_skin extends skin {
  function main_skin($filename) {
    ...
    // Number of members
    list($TMPL['num_total_members']) = $DB->fetch("SELECT COUNT(*) FROM {$CONF['sql_prefix']}_servers", __FILE__, __LINE__);
    list($TMPL['num_members']) = $DB->fetch("SELECT COUNT(*) FROM {$CONF['sql_prefix']}_servers WHERE active = 1", __FILE__, __LINE__);

 // Build the multiple pages menu
    $delimiter = ' | ';  // Set this to whatever you want between links
    ...
    if ($TMPL['num_members'] > $CONF['num_list']) {
      ...
      while ($num > 0) {
        ...
        if ($FORM['start'] == $start) {
          ...
          $TMPL['multiple_pages_menu'] .= "<option value=\"{$start}\">{$end}</option>\n";
        }

        if ($done != 0) { $TMPL['multiple_pages_links'] .= $delimiter; }
        $TMPL['multiple_pages_links'] .= "<font size='5'><b><a title=\"С {$start} - по {$end} сервер\" href=\"index.php?start={$start}\">{$page}</a></b>\n";
    ...
    if ($CONF['featured_member'] && $TMPL['num_members']) {
      $result = $DB->select_limit("SELECT username, url, title, description, banner_url FROM {$CONF['sql_prefix']}_servers WHERE active = 1 ORDER BY RAND()", 1, 0, __FILE__, __LINE__);
