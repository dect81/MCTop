...
class offline extends base {
  ...
  function form() {
    ...
    $alt = '';
    $num = 0;
    #$result = $DB->query("SELECT username, url, title, email, user_ip, serv_ip, serv_port, attemps, success FROM {$CONF['sql_prefix']}_servers WHERE active = 1 and status = 0 ORDER BY (success/attemps*100) DESC", __FILE__, __LINE__);
    $result = $DB->query("SELECT * FROM {$CONF['sql_prefix']}_servers servers, {$CONF['sql_prefix']}_stats stats WHERE servers.id = stats.id and servers.status = 0 ORDER BY score DESC", __FILE__, __LINE__);
...
<form action="{$TMPL['list_url']}/index.php?a=admin" method="post" name="inactive">
...
EndHTML;
      ...
      while ($row = $DB->fetch_array($result)) {
	    ...
	    $TMPL['uptime'] = $TMPL['success']/$TMPL['attemps']*100*100;
	    $TMPL['uptime'] = ((int)($TMPL['uptime'])/100.0);	
	    $TMPL['uptime'] = $TMPL['uptime'].'%';
...
<td align="center"><font color="black">$username [{$TMPL['uptime']}] [{$TMPL['active']}]</td>

<td width="50%">Очков: {$TMPL['score']} <a href="{$url}" onclick="out('{$username}');">{$TMPL['title']}</a></td>

<td align="center"><a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=inactive&amp;u={$username}" title = "{$username}">{$LNG['a_approve']}</a></td>
<td align="center"><a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=edit&amp;u={$username}" title = "{$username}">{$LNG['a_man_edit']}</a></td>
<td align="center"><a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=delete&amp;u={$username}" title = "{$username}">{$LNG['a_man_delete']}</a></td>
