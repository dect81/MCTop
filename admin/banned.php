...
class banned extends base {
  function banned() {
    ...
    $TMPL['header'] = 'Заблокированные сервера';
  ...
  function form() {
    ...
    $alt = '';
    $num = 0;
    $result = $DB->query("SELECT username, url, title, email, user_ip, serv_ip, serv_port FROM {$CONF['sql_prefix']}_servers WHERE active = 3 ORDER BY username ASC", __FILE__, __LINE__);
...
EndHTML;
      ...
      while (list($username, $url, $title, $email, $user_ip, $serv_ip, $serv_port) = $DB->fetch_array($result)) {
...
<td width="100%"><a href="{$url}" onclick="out('{$username}');"><img src="http://mctop.su/top/check/?ip={$serv_ip}&port={$serv_port}&type=1" border="0" alt=""/> {$title}</a></td>
