...
class manage extends base {
  function manage() {
    ...
    $usernames_menu = '';
    $result = $DB->select_limit("SELECT username FROM {$CONF['sql_prefix']}_servers WHERE active = 1 ORDER BY username ASC", 1, 0, __FILE__, __LINE__);
    list($username_start) = $DB->fetch_array($result);
    while ($username_start) {
      $result = $DB->select_limit("SELECT username FROM {$CONF['sql_prefix']}_servers WHERE active = 1 AND username > '{$username_start}' ORDER BY username ASC", 2, $num_list - 2, __FILE__, __LINE__);
      list($username_end) = $DB->fetch_array($result);
      if (!$username_end) {
        $result = $DB->select_limit("SELECT username FROM {$CONF['sql_prefix']}_servers WHERE active = 1 ORDER BY username DESC", 1, 0, __FILE__, __LINE__);
        list($username_end) = $DB->fetch_array($result);
      }
    ...
    list($num_members) = $DB->fetch("SELECT COUNT(*) FROM {$CONF['sql_prefix']}_servers WHERE active = 1", __FILE__, __LINE__);
...
EndHTML;
    ...
    $alt = '';
    $num = 0;
    $result = $DB->select_limit("SELECT username, title, url, email, user_ip FROM {$CONF['sql_prefix']}_servers WHERE active = 1 AND username >= '{$start}' ORDER BY username ASC", $num_list, 0, __FILE__, __LINE__);
