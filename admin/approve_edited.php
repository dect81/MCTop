...
class approve_edited extends base {
  ...
  function form() {
    ...
    $alt = '';
    $num = 0;
    $result = $DB->query("SELECT username, url, title FROM {$CONF['sql_prefix']}_servers_edited ORDER BY username ASC", __FILE__, __LINE__);
...
EndHTML;
	...
	$username_sql = $DB->escape($username);
        list($url, $title) = $DB->fetch("SELECT url, title FROM {$CONF['sql_prefix']}_servers WHERE username = '{$username_sql}'", __FILE__, __LINE__);
...
<td align="center"><a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=approve_edited&amp;c=approve&amp;u={$username}" title = "{$username}">{$LNG['a_approve']}</a></td>
<td align="center"><a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=approve_edited&amp;c=reject&amp;u={$username}" title = "{$username}">{$LNG['a_approve_edited_reject']}</a></td>
</tr>
EndHTML;
        if ($alt) { $alt = ''; }
        else { $alt = 'alt'; }
...
EndHTML;
  ...
  function do_approve($username) {
    ...
    list($url, $title) = $DB->fetch("SELECT url, title FROM {$CONF['sql_prefix']}_servers_edited WHERE username = '{$username}'", __FILE__, __LINE__);
    $url = $DB->escape($url);
    $title = $DB->escape($title);

    $DB->query("UPDATE {$CONF['sql_prefix']}_servers SET url = '{$url}', title = '{$title}' WHERE username = '{$username}'", __FILE__, __LINE__);
    $DB->query("DELETE FROM {$CONF['sql_prefix']}_servers_edited WHERE username = '{$username}'", __FILE__, __LINE__);
  }
