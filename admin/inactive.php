...
class inactive extends base {
  function inactive() {
    ...
    $TMPL['header'] = $LNG['a_inactive_servs_head'];
  ...
  function form() {
    ...
    $alt = '';
    $num = 0;
    $result = $DB->query("SELECT id, username, url, title, email, user_ip, success, attemps FROM {$CONF['sql_prefix']}_servers WHERE active = 2 and username <> 'Helper' ORDER BY username ASC", __FILE__, __LINE__);
...
<form action="{$TMPL['list_url']}/admin" method="post" name="inactive">
<table border ="1" cellpadding="1" cellspacing="1" width="100%">
<tr class="mediumbg">
<td></td>
<td align="center" width="1%">{$LNG['g_username']}</td>

<td width="1%">{$LNG['table_title']}</td>
<td align="center" colspan="6">{$LNG['a_man_actions']}</td>
</tr>
EndHTML;

      while (list($id, $username, $url, $title, $email, $user_ip, $success, $attemps) = $DB->fetch_array($result)) {
...
<td align="center"><font color="black">$username [$uptime]</td>
<td width="100%"><a href="{$url}" onclick="out('{$username}');"><img src="{$list_url}/status/s{$id}/1" border="0" alt=""/> {$title}</a></td>
<td align="center"><a href="{$TMPL['list_url']}/admin/u/inactive/{$username}" title = "{$username}">{$LNG['a_approve']}</a></td>
...
<tr class="lightbg{$alt}">
<td><br/><br/><input type="checkbox" name="u[]" value="{$username}" id="checkbox_{$num}" /></td>
<td align="center"><br/><br/><font color="black">$username</td>
<td width="100%"><br/><br/><a href="{$url}" onclick="out('{$username}');"><img src="{$list_url}/status/s{$id}/1" border="0" alt=""/> Тестовый сервер</a></td>
<td align="center"><br/><br/><a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=edit&amp;u={$username}" title = "{$username}">{$LNG['a_man_edit']}</a></td>
<td align="center"><br/><br/><a href="mailto:{$email}" title = "{$username}">{$LNG['a_man_email']}</a></td>
<td align="center"><br/><br/><a href="javascript:void(0);" onclick="popup('ban_{$num}')">{$LNG['a_menu_manage_ban']}</a>
<div id="ban_{$num}" class="lightbg{$alt}" style="display: none; border: 1px solid #000; position: absolute; padding: 2px; text-align: left;">
<a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=manage_ban&amp;string={$url_url}&amp;field=url&amp;matching=1">URL</a><br />
<a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=manage_ban&amp;string={$user_ip_url}&amp;field=ip&amp;matching=1">User IP</a><br />
<a href="{$TMPL['list_url']}/index.php?a=admin&amp;b=manage_ban&amp;string={$username_url}&amp;field=username&amp;matching=1">Username</a><br />
...
EndHTML;
  ...
  function process() {
    ...
    if (is_array($FORM['u']) && count($FORM['u']) > 1) {
      ...
      $LNG['a_approve_inactive_done'] = $LNG['a_approve_inactive_done'];
    }
    else {
      if (is_array($FORM['u']) && count($FORM['u']) == 1) {
    ...
    $TMPL['admin_content'] = $LNG['a_approve_inactive_done'];
  }
