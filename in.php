...
class in extends in_out {
  function in() {
    ...
    if ($username) {
      list($username_sql) = $DB->fetch("SELECT username FROM {$CONF['sql_prefix']}_servers WHERE username = '{$username}'", __FILE__, __LINE__);
  ...
  function get_username($url) {
    ...
    $username = '';
    while (!$username) {
      list($username) = $DB->fetch("SELECT username FROM {$CONF['sql_prefix']}_servers WHERE short_url = '{$url}'", __FILE__, __LINE__);
